...
python main.py --listen 127.0.0.1 --port 8188   # headless server (on this machine it is started
curl -s http://127.0.0.1:8188/system_stats | head -c 300   # and stopped by hand — §4, and below)
```
...
- ComfyUI git clone at `~/imagegen/ComfyUI`. **It is not running between sessions and it does not
  start at login**: the `com.insignificant.comfyui` LaunchAgent (KeepAlive + RunAtLoad, plist still
  in `~/Library/LaunchAgents/`, logs in `~/imagegen/logs/`) is deliberately left `disabled`, because
  a KeepAlive service cannot be shut down at the end of a session and an idle server costs ~58 GB.
  Start and stop it by hand per §4. The dormant Comfy Desktop app also wants port 8188 — never open
  it while the server is up.
...
| 2026-08-01 | **A border flood cannot key background that the SUBJECT encloses, and the pick gate cannot see what it leaves behind.** 60 of 67 top-down cells trapped plate-coloured background inside a drawn bow, between a sling strap and the fist holding it, or between missiles on a rail; it is invisible on the light-grey contact sheet the human picks from and ships as a glaring pale blob on a dark plate (archers era 1 carried three white paddles where its slings should be). The fix is a second cut at a **much tighter colour tolerance than the flood's** — trapped background is flat and exactly the plate colour, while pale subject material is shaded and reads slightly off it, so 25-against-60 removes the pockets while keeping white missiles and the sanctioned white hull disc, where 60 eats into them. **Re-measure the tolerance per class rather than inheriting it:** the same pass gains nothing on compact single objects with no closed loops and punched 12k px out of a grey bomb's own body, because a grey subject lit from above has highlights within tolerance of a grey plate. |
| 2026-08-01 | **A long subject cannot be both wide and axis-aligned in a square frame; that is geometry, not wording.** Eight seeds across two opposed rounds on one barrier cell gave a clean split — every wide render landed ~40° off axis and every axis-aligned render came out a narrow single file — because pinning a segment's ends to the frame's top and bottom edges makes it span the full height, and modules sized to span it leave no width to spare. Stop wording rounds at that point and read the trade instead: the wide renders were wide because they were tilted enough to show the subject's faces, so the narrow one is often the honest overhead read. **Do state an axis when you need one, but only as a bounded pair of points** (two named end pieces near the top and bottom edges), never as a run — the bounded form landed 4/4 seeds within 0.7°, while "its length running from near the top of the frame down to near the bottom" tapered every wall it touched. |
| 2026-08-02 | **A shared style clause that contradicts one subject in the class is resolved by the model inventing geometry, not by ignoring the clause.** A new prop class carried "a simplified chunky shape" in its tail, which is right for heaps, rims and craters and wrong for the one core asking for a flat burn on the ground: that cell came back as a ring of boulders around an ash bed, faithfully chunky and nothing like what was asked, and it also broke the design ruling that the class must never look like an obstacle. Split the form clause per subject family rather than per class (flat ground marks take "a simplified flat shape … lying flush with the ground with no height or relief") and **probe the family that fights the tail first**, because the cells that agree with the tail pass and tell you nothing. Related, same probe: a prop core must never name the ground it sits on — "burnt onto grey stone" produced stone objects, and the material is the plate's job. |
| 2026-08-03 | **A single solid object with a canonical side silhouette cannot be pulled to an overhead camera by wording; a spread of parts needs no help at all.** One prop class split perfectly along that line: every heap, spill, scatter and ground mark rendered genuinely overhead first try, while a boulder, a slab, a crate, stacked sandbags and a "fallen log lying flat" all came back as three-quarter product shots (the log as a standing tree trunk) through three generations of three different strategies — the unit class's overhead clause, a top-face clause naming the upward surface as the dominant feature, and the battle plates' own "photographed flat from straight above, orthographic" plus a footprint register shift. Only the last fixed anything, and only the two subjects that were already becoming multi-part (a sandbag row said to lie in one single layer, a crate rewritten as loose boards with no box left standing). **The reliable lever is a subject change: make the prop several things instead of one thing**, because a rock has one silhouette the model knows it by and from overhead it does not look like that silhouette, while a scatter of stones has no canonical profile to lose. Recognise the family at the FIRST failure and take the subject question to the human then, not after three rounds. |
| 2026-08-03 | **A pick gate can come back as a design change, so build the picks file to hold one.** A gate sent up as "one seed per row" returned seed picks, two outright drops, *and* a rule the class did not have when it was commissioned: every prop got a barrier tier, which turned a decoration class into a gameplay entity and inverted a fortification rule elsewhere in the corpus. Three practical consequences. **Widen the pick schema before the gate, not after**: a `picks: {id -> seed}` map could not express "use all four seeds as variants", "drop this one", or "this one blocks 2-3 shots", and rewriting the freeze script's contract after approval is the worst moment to do it. **A rule value living in an art table has to say so in the table**, in the header comment and in the registry it feeds, or the next person edits a tier thinking it is metadata. And **the art is already rendered when the rule arrives**: this class tied on-field size to the barrier tier, but the sprites had been sized for looks, so the tier drives the view's scale and the sprite's pixels are not evidence of anything. |
