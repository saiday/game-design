...
2. **Buildings & units ×6 eras** — the volume class. Per building line: era N approved sprite
   ...
   fortifications and enemy tiers, 15 lineage lines × 4 seed chains): **all six eras CLOSED,
   pick gate next** (era 1: 60 cells after 7 re-roll rounds; era 2: 48 cells after 4 rounds
   incl. one rebuilt e1 root; era 3: 54 of 56 cells after 4 rounds, cavalry rebuilt as a
   txt2img lineage seam, artillery chain 83 + anti_air chain 84 deliberately left unresolved
   at 3 clean chains each; era 4: 53 clean of 58 after 4 re-roll rounds, 5 chains closed
   short; era 5: 49 clean of 53 after 2 re-roll rounds, all 15 lines at ≥3 clean,
   archers/enemy_mid/enemy_hard/bomber era-5 rebuilt as txt2img roots; era 6: 47 clean of 49
   after 2 re-roll rounds, first-pass 40/49 = 82% via audit-first lever pre-assignment,
   cavalry/holy_warriors/elite_forces/enemy_hard era-6 as txt2img roots, enemy_weak short at
   2 + privateers short at 1)** (`phase3_units_wave.py` + `phase3_unit_chains.json`, same
   era-gated machinery as buildings; all eras done — lineage contact sheets, manifest sync,
   and the human pick gate are the remaining steps).
   Era-3 wording is authored under the positive-slot rule (§14 2026-07-18): **never name a
...
| 2026-07-18 | **Units era 3 CLOSED (54/56 cells, 4 re-roll rounds) — the era where lines change CATEGORY rather than evolve, which is where inheritance fights hardest. Two levers were isolated by controlled comparison, and one of them reverses the previous era's advice.** (1) **NEVER NAME A BANNED OBJECT — describe the desired slot positively.** Round 2 was an accidental controlled experiment: 3 prohibition-style fixes vs 3 positive-description fixes, authored the same day against the same defect class. All 3 positives fixed their cell; all 3 prohibitions failed and 2 got measurably WORSE — artillery "plain **unlit** ... does not glow" turned 1 glowing tube into 2, shield_wall "**no** arrows and **no** blades anywhere" turned 4 arrows into 7 projecting blades plus sparks, infantry "**nothing else** carried" displaced the inherited spear into a floating blade + curled hook overhead. At cfg 1 with no negative-prompt lever the banned tokens still condition the image; "no blades" reads substantially as "blades". The v3 rewrites (fill both hands, "an unbroken smooth masonry face", "its top an open flat circle of bare stone paving") fixed what the prohibitions had inflamed. This SUPERSEDES the era-1/2 habit of pinning counter-wording, and it explains the older backgrounds finding that denial-only pins ("empty", "deserted") keep breeding occupants — same mechanism, now demonstrated head-to-head. (2) **Wording can suppress an artifact the parent image does NOT contain, but not one it does; that case needs a structural lever.** anti_air's "does not glow" worked (its parent had no glowing object) while artillery's identical phrasing failed 5× (its e2 prompt literally specifies "a glowing red linstock"). Added `P3U_DENOISE` env override: at 0.70 infantry:84 and shield_wall:83 came back clean after failing at 0.55 under IDENTICAL wording — the cleanest available proof that the residue is parent-borne, not prompt-borne. Cost of 0.70: it also weakens adherence to the sprite framing convention the parent carried (anti_air:84 rendered a blue sky background and a bottom-cropped tower — §8 framing reject), so treat 0.70 as a targeted rescue, not a default. (3) **When neither lever works, break the lineage.** Cavalry went 16 cells / 4 rounds / 3 wordings / 0 clean: v2 fixed the fantasy horned chamfrons 8/8 but every seed then filled the composition's empty canvas with an unattached invention (floating shield, standing sword, rump finials, curled hook, a third horse). Human-approved fix: regenerate era 3 as **txt2img**, accepting a permanent lineage seam — 4/4 clean on the first round. Added `P3U_T2I_LINES` env force plus a `"root": true` tag in chain state so the seam stays visible to sheets and manifest; `phase3_unit_sheets.py` now also renders `"rejected": true` cells with a red REJECTED overlay so a §8-rejected cell can never read as a pickable candidate. (4) **Know when to stop: 3 clean chains is enough for a gate that picks ONE lineage per line.** artillery:83 (5 failures) and anti_air:84 (4 failures) were deliberately left unresolved rather than seamed — a seamed 4th option would almost certainly lose to 3 seamless ones at the pick gate, so the rolls would have bought nothing. (5) **Era-N subjects must be audited against era-(N-1) BEFORE the wave**, not after: 5/5 lines that replaced a dominant e2 element inherited it (e2 spears → a shaft across every e3 swordsman, e2 "roofed shooting platform" → a tiled pavilion on a stone tower, e2 timber sled → under a cannon specified as wheeled). Corollary: **an unnamed slot is where the parent's version persists by default** — 4 era-4 subjects leave headgear unnamed, so name every slot the parent fills, even when the answer is "bare head". |
| 2026-07-19 | **Units era 4 CLOSED (53 clean of 58, 4 re-roll rounds, 5 chains closed short) — "carry-through" splits into THREE mechanisms with three different levers, and the §8 review moved to parallel subagents.** (1) **INHERITANCE FROM THE PARENT IMAGE → denoise `P3U_DENOISE=0.70`.** Cleared the inherited object 8/8 (engineers' era-3 standing plank wall 4/4, enemy_mid's kite shield 2/2, enemy_hard's spiked greathelm + flail 2/2) and — new — cleared an inherited POSE 4/4 where two rewrites had failed (holy_warriors' figures kept swinging muskets overhead in the era-3 mace silhouette; **object identity and pose inherit SEPARATELY**, so wording that replaces the noun leaves the verb untouched). The 0.70 framing cost is NOT universal: it scales with how wide the composition already sits, breaking infantry (rank of three, horizontal muskets) 4/4 and enemy_mid 1/2 while engineers and enemy_hard (compact) broke 0/6. (2) **PROMPT-INDUCED SYMMETRY → break the geometry, not the attribution. NEW CLASS.** "a rank of" + "held level" + three identical figures put three identical barrels on one line and the model merged them into ONE bar spanning the whole rank with a single bayonet; the same artifact appeared as engineers' one plank shared across two shoulders and holy_warriors' single barrel gripped by all three figures including the bearer. **Present identically at 0.55 AND 0.70 — that invariance is what proves it is not inherited**, and it is self-inflicted by the prompt's own symmetry (it retroactively explains an era-3 infantry cell misfiled as era-2 carry-through). Naming whose hands hold the object does NOT work (engineers v3 "with both of his own hands" still spanned two figures); only geometry does — stagger the line, stand the object upright, tuck it under one arm. (3) **BLANK-SURFACE INSIGNIA PRIOR → occupy with an OBJECT, not with paint, and expect it to migrate.** Five hits on four surfaces this era: elite_forces' bearskin cap fronts (gold bugle horn, cross pattée), privateers' sleeves (rank chevrons), enemy_hard's pauldron (ring-and-flame badge), then privateers' pennant (saltire, a literal Swiss flag, a Soviet-style star). A cockade (an object on the cap) fixed elite_forces 4/4; a painted circle on privateers' sleeve OSCILLATED across four attempts — blank→chevrons, "plain white circle"→red-ringed roundel, "solid flat white disc"→chevrons again, plain leather strap→the strap itself converted into an armband bearing a device. Naming the armband plainly ("of unbroken solid colour", the phrasing that had fixed the same line's pennant) finally produced solid armbands 3/3 — **and the prior immediately jumped to the pennant, a surface that had passed clean in two earlier rounds.** Standing rule: when a fix DISPLACES an artifact into the same family rather than removing it, that is a stable model preference, not a wording bug — name the preference in benign form early instead of trying a fifth occupant, and accept that a strong prior may simply migrate. (4) **Audit era N against era N-1 BEFORE the wave, and mind `START_ERA`.** The audit indexed `LINES[line][3]`, which is era 4 only for lines starting at era 1; five lines start later (elite_forces/artillery at 2, holy_warriors/privateers at 3, bomber at 4), so three lines were never audited and one edit landed on era 5. The yield split cleanly: audited lines 14/26 first-pass (54%), un-audited 4/15 (27%), and the two worst un-audited lines failed on exactly the carry-through an audit would have named. A line whose `START_ERA` equals the wave era is a **txt2img root with no parent** — bomber's failures were pure count drift, fixed by "exactly two ... standing side by side" (the construction that also fixed cavalry's horse count and anti_air's barrel count). (5) **§8 review by parallel subagents works, at ~1/200th the orchestrator's context cost** (each reviewer spends 50-110k tokens and returns a ~300-token verdict table), and reviewers reach for objective tests an eyeball skips — per-border pixel diffs to prove a musket crosses the canvas edge, pixel-connectivity to tell an enclosed hole from an edge notch. Two hazards, both authored by the orchestrator, both now fixed in `assets/pipeline/review-brief-units.md`: **concurrent reviewers MUST get a private crop directory and stem-named files** (ten agents sharing one scratchpad picked seed-only names like `s84_top.png`, overwrote each other and reported defects from other lines' cells — that whole round was discarded and re-run), and **an over-broad rule induces false rejects** (a duplicate-weapon rule written off infantry's genuinely broken cases rejected well-formed slung kit; a costume deviation was read as subject mismatch). The brief now carries a REJECT/PASS table for duplicate weapons, an explicit "costume deviation is PASS-with-note", and per-line anti-false-positive notes. Corollary for the orchestrator: **treat a surprising reviewer verdict as possible instrumentation error before acting on it** — a false-positive round briefly "falsified" the correct two-objects-inherit theory. (6) Closed short (the era-3 rule held: 3 clean chains is enough for a gate that picks ONE lineage per line): enemy_mid 84 (3 framing breaches at 0.70), holy_warriors 82 (headless standard bearer on 2 consecutive seeds). privateers closed short at ONE chain by human decision after the migrating prior — materially weaker, since eras 5-6 for that line all descend from chain 82. **Ops:** editing `phase3_unit_chains.json` while a driver is running loses the edit — the driver dumps its stale in-memory dict on exit and clobbered a rejection flag; `pgrep -f phase3_units_wave` before every state edit, not just before every launch. |
| 2026-07-19 | **Units era 5 CLOSED (49 clean of 53, 2 re-roll rounds, all 15 lines at ≥3) — a FOURTH carry-through mechanism the audit cannot reach: at denoise 0.55, img2img preserves the parent's OBJECT CATEGORY and FIGURE COUNT, and the fix is the lineage lever, not wording.** Era 5 is where 4 lines change subject entirely (cavalry/holy_warriors/enemy_hard → tank, bomber → aircraft) and several drop figure count (archers 3→2, engineers 3→2 escorts, enemy_mid keeps its count but inherits the parent's per-figure weapon count). Full auditing was done up front — and first-pass yield still landed at 21/53 (39.6%), just UNDER era 4's un-fully-audited 43%. The honest read: **auditing fixed everything it targeted** (infantry's historical collinear-rifle merge did not recur, 3/4; structures transformed cleanly, shield_wall 4/4, trench eventually 4/4) but wording cannot beat a categorical or count transform at 0.55 — every wholesale-0 line (archers, bomber, enemy_hard, engineers, enemy_mid all 0/4 first pass) failed on category/count, not on any nameable slot. (1) **0.70 clears an inherited OBJECT or a residual FIGURE on a same-count subject** — round 1 at 0.70 cleared artillery's two walking gunners, cavalry's horse-leg walker + double barrel + floating braid, anti_air's welded flak barrel + skull-mutated insignia, privateers' inherited pennant, and holy_warriors' fused human legs (each 1/1 or better). (2) **0.70 does NOT reliably clear a COUNT-composition defect** — archers 3→2 and enemy_mid two-weapons-per-figure each cleared only 1 cell by seed luck across BOTH 0.55 and 0.70 (archers 0/4 then 1/4; enemy_mid 0/3 then 1/3), and enemy_hard's inherited walker-legs persisted 2/4 even at 0.70. This is the same class as bomber's balloon (a silhouette that shares nothing with the target aircraft), which no denoise can dissolve. **Lever: make era N a txt2img root** (the cavalry-e3 precedent), which removes the parent entirely — round 2 as roots went archers 3/3, enemy_mid 2/2, enemy_hard 3/3, bomber 2/2 (12/12 clean). Diagnostic that tells the two apart before spending a round: if the defect is the parent's *count or whole silhouette* rather than a *part welded onto an otherwise-correct subject*, skip 0.70 and go straight to a root. Roots are tagged `"root": true` and coexist within a line with img2img chains (the clean round-1 chain stays a descendant); the pick gate takes one lineage per line either way. (3) **The insignia prior migrates on a subject change too, and the positive-slot rule still answers it** — holy_warriors' human legs cleared at 0.70 but the open barrel-decoration slot ("votive ribbons") rendered a golden CRUCIFIX (the line's reject class); naming "small golden tree-shaped charms" filled the slot with the sanctioned emblem, clean on the next roll. (4) **Structure lines (shield_wall, trench) img2img cleanly through a material change** (sandbag wall → concrete bunker, earth trench → concrete anti-tank ditch) because neither count nor category of the SUBJECT changes — only its material — so they need no special lever; their only rejects were pure hallucinations (wagon wheels, a wall-shadow face) that a plain seed re-roll fixed. (5) **Ops:** a re-review subagent died mid-round on the session token limit and returned a partial verdict; re-dispatching the SAME line/cells with a fresh crop dir recovered it with no state damage (the reviewer never touches chain state) — treat a `failed` reviewer as a re-dispatch, not a re-roll. 4 cells stayed §8-rejected in state (elite_forces 84, enemy_weak 84, engineers 84, infantry 83) because their line hit 3 clean without them; flagged `"rejected": true` so the sheets red-overlay them and the manifest can't pick them. |
| 2026-07-20 | **Units era 6 CLOSED (47 clean of 49, 2 re-roll rounds) — the audit-first payoff lands: FIRST-PASS 40/49 = 82%, roughly double era 4 (43%) and era 5 (40%), by pre-assigning the lineage lever per transition type instead of running everything at 0.55 and re-rolling.** Era 6 is the final (sci-fi) era; the era-5 diagnostic was applied BEFORE the wave: classify each era5→era6 transition, then (a) **txt2img root** for a category/count reversal, (b) **denoise 0.70** for a dominant parent object to shed at preserved count, (c) **0.55** for a same-category evolution. The buckets validated the split cleanly: roots 13/14 first-pass (cavalry 4/4, enemy_hard 4/4, holy_warriors 3/3, elite_forces 2/3 — the one miss a single-mech count, not a category failure), 0.70 lines 9/9 (infantry shed its troop truck 3/3, engineers shed its yellow vehicle 3/3, anti_air shed its tracks→turret 3/3), and the 0.55 same-category lines 18/26, where every failure was a small-defect class (a floating glyph, an inherited propeller, an insignia glyph), never a category/count miss. This is the direct evidence the audit-first method pays off ONLY when the pre-classification is correct: era 5 audited wording but not lineage lever and gained nothing; era 6 audited the lever and doubled first-pass. (1) **The blank-surface insignia prior is the era's dominant re-roll driver and confirms the era-4 migrating-prior rule a second time.** enemy_mid's UNNAMED shoulder pauldron drew an invented orange glyph 6/6 across two wording rounds — "smooth unbroken matte black shoulder pads" failed exactly as "plain white circle" had on privateers' sleeve, and the glyph migrated in family (hexagon ring → chevron/caret → red roundel → S-scribble). The fix that finally held 3/3 was NOT a stronger prohibition but a benign SANCTIONED occupant: "a single plain orange band across each shoulder pad" (the same simple horizontal shape enemy_hard's turret stripe uses clean). enemy_weak repeated the lesson in miniature: naming plain chest studs cleared the chest but the glyph JUMPED to the shoulder as a lettered armband — so it was closed short at 2 rather than chasing the mark around the figure. **Occupy the surface the prior wants with a benign sanctioned mark; do not try to keep it blank, and accept migration as the stop signal.** (2) **A dropping-payload subject invites a SECOND unattached object in the empty canvas.** bomber's "bomb bay open with a single guided bomb dropping" twice grew a second free-floating object beside the real bomb (a yellow arrow glyph, then a small ghost aircraft) — the open sky next to a legitimately falling object is the carrier. Seed re-roll cleared it; worth a dedicated zoom on the empty canvas around any "dropping/falling" subject. (3) **Structure lines stayed the cheap, reliable bucket** (shield_wall concrete→composite-hex wall 4/4, trench ditch→automated-turret wall 4/4 first-pass) — a material/feature change at constant subject count carries no inheritance tax, confirming era 5's structure finding. (4) **Ops, again:** the entire 15-line first-pass review batch was dispatched, then 10 of 15 reviewers died mid-run when the session token limit reset at 3am; re-dispatching the 10 failed lines (unchanged prompts, cleared crop dirs) recovered them with zero state impact, since reviewers never touch chain state — the same "a failed reviewer is a re-dispatch, not a re-roll" rule from era 5, now at 10× scale. Two cells stayed §8-rejected in state (bomber 82 floating object, enemy_weak 82 migrated glyph). |
