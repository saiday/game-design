...
| 2026-07-08 | **pixelize.py verified** (ComfyUI's venv python has Pillow; system python3 doesn't — run pipeline scripts with `~/ComfyUI-Installs/ComfyUI/standalone-env/bin/python`): 1024² → 64×64 with an 8-color stand-in palette → 0 off-palette pixels, @8x review copy written. Real master palette is the Phase 1 human gate. |
