...
| `insignificant-game/docs/explore-topdown-motion-demo.html` | `ask` | 1.2 MB, **94% base64 sprite payload** and only ~70 KB of hand-authored JS/CSS. Reading it whole is ~300k tokens and buys almost nothing. |

If the human does ask you to work on the motion demo, don't read the raw file. Its rules are pinned
to ADR-0006/0007 by `insignificant-game/docs/tools/check_motion_demo.js`, its sprite block is
regenerated by `docs/tools/build_motion_demo.py` (hand-edited JS and CSS survive a re-run), and this
gives you the hand-authored ~18k tokens without the payload:
...
print(re.sub(r'data:image/[a-z]+;base64,[A-Za-z0-9+/=]+', 'data:<stripped>', t))
