...
| The documentation architecture itself | `docs/doc-map.html` (human-facing doc map; update it in the same change whenever you add, move, re-role, or delete a doc, and bump its footer. A doc joining a cluster it already maps (setting doc, ADR, audit) needs no new row; a new kind of doc does). Forgetting a row is a Part A failure now, not a silent hole — `insignificant-game/docs/tools/check_design_graph.py` enforces coverage. The `@GRAPH` block is generated; don't hand-edit it |
| Checking the design corpus, doc↔code metadata, or doc-map coverage | `insignificant-game/docs/tools/check_design_graph.py` (deterministic, no LLM, no deps; commands and the full invariant list in `insignificant-game/docs/dev-loop.md`) |
...
## Gated files (do not open unless the human asks for them by name)

Enforced at the harness level in `.claude/settings.json`, because a documented expectation that
nothing checks is not an expectation. The rules only cover the `Read` tool, so the reason each file
is gated is written here too: `cat`, `grep`, and `rg` can still reach them, and you are what stops
that.

| File | Rule | Why |
|---|---|---|
| `docs/prompts.md` | `deny` | Human-only prompt log. Agents never read it. It churns every turn, so stage specific paths when committing. |
| `insignificant-game/docs/explore-topdown-motion-demo.html` | `ask` | 1.2 MB, **~95% generated payload** (base64 sprites plus the embedded timeline fixture) and only ~70 KB of hand-authored JS/CSS. Reading it whole is ~300k tokens and buys almost nothing. |

If the human does ask you to work on the battle replayer, don't read the raw file. It holds **no
rule code**: it replays the timeline `core/battle.gd` exported into
`insignificant-game/docs/fixtures/battle_timeline.json` (read that instead when you need to know
what happens). `insignificant-game/docs/tools/check_motion_demo.js` checks the renderer, fixture
freshness, and that no rule code came back; both generated blocks are regenerated by
`docs/tools/build_motion_demo.py` (hand-edited JS and CSS survive a re-run). This gives you the
hand-authored ~18k tokens without the payload:

```bash
cd /Users/saiday/projects/game-design/insignificant-game && python3 -c "
import re, pathlib
t = pathlib.Path('docs/explore-topdown-motion-demo.html').read_text(encoding='utf-8')
t = re.sub(r'data:image/[a-z]+;base64,[A-Za-z0-9+/=]+', 'data:<stripped>', t)
print(re.sub(r'const TIMELINE = .*', 'const TIMELINE = <see docs/fixtures/battle_timeline.json>;', t))
" > /tmp/motion-demo-source.html
```
...
- **Doc ↔ code metadata:** every design doc's frontmatter carries a `code:` list mapping it to
  ...
  change.
- **The corpus is a connected graph, declared in frontmatter.** Each system doc's `graph:` block
  names what feeds it, what it feeds, and the `GameState` fields it reads and writes — the
  machine-readable twin of the prose 被誰餵/餵給誰 intro line. Edit the two together.
  `insignificant-game/docs/tools/check_design_graph.py` resolves every edge and reports
  asymmetries; it never invents an edge to make itself pass, so a warning is a real question.
...
surface plausibility. The *mechanical* half of the disconnect hunt is now automated — run
`check_design_graph.py` first and let skeptics spend their effort on whether an edge makes
sense, not on whether it was written down on both ends.
