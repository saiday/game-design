#!/usr/bin/env python3
"""Embed the sprites and the exported battle timeline into explore-topdown-motion-demo.html.

The page is deliberately a single self-contained file: sprites live in it as base64 data URIs
and the timeline fixture as inline JSON, so it can be double-clicked from anywhere. Relative
paths would add a silent-404 mode the moment the HTML travels, and a `fetch()` of the fixture
would fail outright over file://. This script is what makes that practical: it keys the plain
grey render background to transparent, trims, downscales, quantises, inlines the fixture, and
rewrites ONLY the marked blocks plus the footer credits, so hand-edited JS and CSS survive
re-runs.
...
Input  : assets/exploration/topdown-demo/<class>_e<era>.png  (exploratory raws, not pipeline
         ...
         docs/fixtures/battle_timeline.json  (written by tools/export_timeline.gd)
Output : docs/explore-topdown-motion-demo.html, between the @SPRITES / @TIMELINE / @CREDITS
         markers.

Part A re-runs the exporter and this script and fails on a diff, so a rule change that moves
the timeline cannot leave the page replaying a stale battle (docs/dev-loop.md).
"""
...
FIXTURE = os.path.join(GAME, "docs", "fixtures", "battle_timeline.json")

SPRITES_BEGIN, SPRITES_END = "/* @SPRITES-BEGIN */", "/* @SPRITES-END */"
TIMELINE_BEGIN, TIMELINE_END = "/* @TIMELINE-BEGIN */", "/* @TIMELINE-END */"
...
def main() -> None:
    ...
    with open(HTML, encoding="utf-8") as f:
        html = f.read()
    for marker in (SPRITES_BEGIN, SPRITES_END, TIMELINE_BEGIN, TIMELINE_END,
                   CREDITS_BEGIN, CREDITS_END):
        if marker not in html:
            sys.exit(f"marker missing from the HTML: {marker}")

    # The timeline block: the exported fixture verbatim, minified. Compact separators only —
    # nothing here reformats or filters the events, so a diff against the fixture is meaningful.
    if not os.path.isfile(FIXTURE):
        sys.exit(f"no timeline fixture at {FIXTURE}\n"
                 "  run: $GODOT_BIN --headless --path . -s tools/export_timeline.gd")
    with open(FIXTURE, encoding="utf-8") as f:
        fixture = json.load(f)
    timeline = (TIMELINE_BEGIN
                + "\n// generated by docs/tools/build_motion_demo.py from "
                  "docs/fixtures/battle_timeline.json\n"
                  "// (produced by tools/export_timeline.gd; do not hand-edit)\n"
                  "const TIMELINE = "
                + json.dumps(fixture, ensure_ascii=False, separators=(",", ":"))
                + ";\n" + TIMELINE_END)
    html = re.sub(re.escape(TIMELINE_BEGIN) + r".*?" + re.escape(TIMELINE_END),
                  lambda _: timeline, html, flags=re.S)
    rounds = sum(len(era["rounds"]) for era in fixture["eras"])
    print(f"timeline: {len(fixture['eras'])} eras, {rounds} rounds embedded")
