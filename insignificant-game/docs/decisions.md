...
## W15.2/W15.3 gaps: the other two scenes

| Gap | Decision | Where |
|---|---|---|
| Where a route node stands, and where a battlefield prop stands — neither is in the core, and neither may enter it | **Seeded on a view-local `RandomNumberGenerator`**, from `run_seed` + generation (+ battle type). Never `state.rng`: drawing a map or dressing a field must not consume a track, or LOOKING at the game would change the run. Same division of labour ADR-0010 already set for scatter | `view/route_scene.gd`; `view/battle_scene.gd` |
| What 迷霧 looks like, given the corpus says it is 畫面語言 and not only a rule | A haze **behind** the marker, never instead of it: an unknown node is visibly THERE and unreadable. The 世界地圖 upgrade lifts the FACE, not the fog (core already decides that in `face_shown`) | `view/route_scene.gd` `_place_marker` |
| Which way a 正規軍 sprite faces, given both camps field the same roster | The flip is a function of **art and side, not side alone**. Player LINES are authored right-facing and the three anonymous enemy tiers left-facing (`review-brief-units-topdown.md` §1), so a 正規軍 on the enemy side mirrors and a 勸降 defector turns around while keeping its tier art | `view/battle_scene.gd` `_mirrored` |
| Which projectile an attack draws — `asset_paths.gd`'s own comment claimed "the core resolves an attack's ammo from the card", and nothing did | `PROJECTILE_BY_FORM` in the registry, per line per era, `&""` where the form closes to melee instead. Pure art bookkeeping (unlike `SCATTER.barrier`, **no rule reads it**). The ammo is then chosen by the attacker's **row**, not by the event's label — a label repeats, and an anonymous tier's grade says nothing about its weapon. Reading the label is what had era-1 mobs firing bullets | `core/data/asset_paths.gd`; `view/battle_scene.gd` `_throw_at` |
| How long a 盾陣 is drawn, given 一張卡就是一段牆 says it spans the row's frontage and not the field | **The frontage of the row it screens**, at the sprite's own aspect (the barriers ship top-to-bottom precisely so the view never rotates or distorts them). Spanning the whole band stretched a 1:2 timber wall into a 1:16 pole. An empty 遠程列 still gets its wall — a screen with nobody behind it is legal and paid for — so that case falls back to the middle of the band | `view/battle_scene.gd` `_row_frontage` |
| Whether a skipped playback still applies its events | **Yes to the state, no to the animation.** A round carries hundreds of events, and spawning a tween and a sprite for each is how a fast-forward becomes a stall — Part B's watchdog caught it. `_silent` applies what events changed and draws none of it | `view/battle_scene.gd` `finish_playback` |
| Newly arrived wave units had no `uid` at the boundary: waves arrive at the END of `end_round`, after the sweep that hands identities out | `_assign_uids` runs **again after `_arrive_waves`**, so the contract's promise ("every unit and fort on the field carries the same uid its events use") holds at the boundary the view actually reads. Found by W15.3, fixed in core, and asserted at the boundary rather than mid-round | `core/battle.gd` `end_round`; `test/battle_test.gd` |

## W16 gaps: teaching the bot to build cover

The bot had skipped `class == &"fortification"` since W14, so every gap here is about *when* a
heuristic should spend 軍費 on a card the design describes only from the player's side. None of
these change a rule; they pin the auto-player's reading of one.

| Gap | Decision | Where |
|---|---|---|
| Where the fort decision goes, given the plan said "a fort branch in `_pick_deploy`" | **A separate pass after the unit race, not a branch inside it.** `_pick_deploy` is called from a loop that fields units until 攻+血 reaches parity, and a 工事卡 carries neither — a parity loop asked to buy one would never stop, or would stop for the wrong reason. Cover rides on top: units first (so the W14 tempo policy stays the dominant term and the batch delta is attributable to the addition), then one fort, then one engineer | `core/sim.gd` `_field_support` |
| When 3 軍費 for a 盾陣 is worth it | When **both halves of the rule are on the field**: a living unit of ours in the 遠程列 to screen, and a living enemy in the 遠程列 to absorb fire from. 近戰 walks around a wall and 空襲 comes over it (ADR-0010), so neither is a reason to buy one. Deliberately written as a rule reading and not an engine reading — ranged focus-fire happens to run in deploy order, and a bot tuned to that would measure the target picker instead of the wall | `core/sim.gd` `_wall_wanted` |
| Which fort first, when both are wanted and 同場上限 is 2 | **防空飛彈 before 盾陣.** 空域 is the only threat the tempo policy cannot answer by accident (it fields the cheapest unit, so it meets bombers with 弓箭團 only when the cheap 步兵團 are spent); a wall answers fire the army can already trade with | `core/sim.gd` `fort_pick` |
| Whether to replace a disabled wall with the second slot | **No, while anything can stand it back up** — an engineer on the field or one still in the deck. A wreck is a slot already paid for and forts are never removed (ADR-0007), so the second slot is the last one the battle will ever have; repairing costs 2 軍費 and re-rolls a fresh budget, replacing costs 3 and burns the slot | `core/sim.gd` `_wall_wanted` |
| When the bot fields a 工兵團 for repair reasons | **As soon as it owns any fort and none is alive on the field.** Without one, a 工事卡 is a one-shot item: nothing else in the game restores it. In practice the rule rarely fires — 工兵團 costs 2 軍費, the same as 步兵團, so the tempo policy already fields it — and it is the backstop for the battles where it does not | `core/sim.gd` `engineer_pick` |
| Whether cover needs a riot case, given 鎮壓的手段有代價 | **No.** 工事卡 are class `&"fortification"` and 工兵團 is `&"personnel"`, so neither trips `mechanical_played` and neither costs 幸福 −15. Cover is riot-safe by construction; adding a special case would have implied a rule that is not there | `core/sim.gd` `_field_support` |
| Whether the bot may disband its last 工兵團 for population | **No, while a 工事卡 sits in the deck.** 工兵團 is disbandable personnel like any other (+2 人口) and the population engine eats the deck in order, so without a guard the bot could trade away the only repairer it will ever have. Protected exactly while it has something to repair — no fort in the deck, no protection | `core/sim.gd` `_is_last_repairer` |
| Where the bot's fort counters live, given `GameState` already carries `unrest_triggered_total` | **Not on GameState.** They describe how this auto-player played, not what the game is, so `Sim.run` returns a `telemetry` dictionary and `balance_batch.gd` folds it into each row. Deploy counts come from the bot's own decisions; absorption, suppression, repair and shootdown counts are read off the timeline contract, which is the same source the two replayers use | `core/sim.gd` `new_telemetry` / `_count_fort_events` |
| How to tell "the bot never had the card" apart from "the bot chose not to play it" | Three **opportunity** counters beside the deploy counts (`battles_wall_held`, `battles_battery_held`, `battles_air_faced`). Without them a low deploy count reads as a policy failure; with them the 防空飛彈 result reads correctly as 1% of battles ever containing a living enemy aircraft | `core/sim.gd` `new_telemetry`; `docs/balance-report.md` §fort delta |
