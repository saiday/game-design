...
| Which axis a 盾陣 sprite is authored on (on the field a wall lies across the lane, top edge to bottom edge, and asking the model for that axis by name tapered the segment to a vanishing point along its own length for three rounds) | **On the field every era reads top to bottom; the render axis is whatever the model held straight, and the view supplies the rotation.** Human ruling. The renderer is never told an axis by name — it is told "a straight row of identical modules, end-capped at both ends, constant width", which is what finally produced a straight wall — so the axis is an outcome to be measured per approved sprite, not a promise the prompt makes. Measured on the approved set: e1/e3/e4/e6 land top-to-bottom already, e5 needs a clean 90°, and e2 renders diagonal and needs ~44°, which resamples it and swings its shading. e2's disposition (rotate, re-roll onto the axis, or accept diagonal) is open | measurements in `assets/pipeline/phase3_units_topdown_picks.json` §`barrier_render_axis`; `assets/pipeline/review-brief-units-topdown.md` §1; the rotation itself lands in `view/` at W15 |
