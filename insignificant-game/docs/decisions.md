...
| Which axis a 盾陣 sprite is authored on (on the field a wall lies across the lane, top edge to bottom edge, but a segment asked for on that axis tapered to a vanishing point along its own length for three rounds) | The render ships the segment running **left to right**; the **view rotates it 90°** at draw time, so every era still reads across the lane. Human ruling, and it costs nothing: the axis is a transform, and authoring on the axis the model can hold straight is what finally produced a straight, constant-width, both-ends-finished wall | `assets/pipeline/review-brief-units-topdown.md` §1; the rotation itself lands in `view/` at W15 |
