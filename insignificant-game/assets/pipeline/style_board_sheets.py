...
def make_cell(img_path: str, label: str, font: ImageFont.ImageFont) -> Image.Image:
    ...
    if img.width > CELL_W - 2 * PAD or img.height > CELL_H - 2 * PAD:  # card art can overflow
        img.thumbnail((CELL_W - 2 * PAD, CELL_H - 2 * PAD), Image.NEAREST)
...
def main() -> None:
    ...
    for recipe, recipe_desc in RECIPES.items():
        header = 48
        sheet = Image.new("RGB", (CELL_W * len(SUBJECTS), header + (CELL_H + LABEL_H) * len(PALETTES)), (24, 24, 24))
        ImageDraw.Draw(sheet).text((PAD, 12), f"{recipe}: {recipe_desc}  (rows: {' / '.join(PALETTES)})",
        ...
        for row, palette in enumerate(PALETTES):
            for col, subject in enumerate(SUBJECTS):
                ...
                sheet.paste(make_cell(dst, label, font), (CELL_W * col, header + (CELL_H + LABEL_H) * row))
