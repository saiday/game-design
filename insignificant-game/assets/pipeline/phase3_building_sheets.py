# phase3_building_sheets.py — Phase 3 building lineage review sheets (cookbook §9): one row per
# seed chain, one column per era, raws as generated — the human judges cross-era coherence along
# the row and picks a whole lineage per line. Cells come from phase3_building_chains.json (the
# era-gated wave state), so re-rolled cells appear under their bumped seeds. The frozen pilot
# line (food) keeps its original fixed-stem sheet. Run with the ComfyUI venv python from
# assets/pipeline/ after the last wave. Usage: phase3_building_sheets.py [line ...]
import json
import os
import sys
...
from phase3_buildings_batch import LINES, SEEDS, START_ERA
...
def main() -> None:
    with open("phase3_building_chains.json") as f:
        state = json.load(f)
    ...
    for line in (sys.argv[1:] or list(state)):
        ...
        ImageDraw.Draw(sheet).text(
            (PAD, 12), f"Phase 3 buildings [{line}] — era lineage chains, era {eras[0]}..{eras[-1]} left to right",
