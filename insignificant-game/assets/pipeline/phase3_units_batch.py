...
LINES = {
    ...
    "holy_warriors": [
        ...
        "two identical musketeers in white coats, each with both hands on a long musket held level with a plain wooden stock and a straight steel barrel, a single golden tree emblem on each chest, beside a standard bearer gripping the pole of a white banner painted with a single large golden tree",  # 神權火槍旅 (v2: era-4 audit, MISSED in the first pass (line starts at era 3). The era-3 flanged mace survived into 4/4 seeds and no musket rendered at all — fill both hands with a concretely described musket (plain wooden stock, straight steel barrel) so the mace has no slot. Banners were clean: single tree, no lettering)
    ...
    "enemy_mid": [
        ...
        "two identical renegade soldiers in ragged dark gray uniforms and battered peaked caps, each with both hands on a bayoneted musket, mismatched armor plates strapped over their chests",                   # 中 (v2: e3 audit — name the headgear slot and fill both hands so e3's kite shield and battleaxe have nowhere to go)
