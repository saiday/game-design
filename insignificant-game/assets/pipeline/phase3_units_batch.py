...
LINES = {
    "infantry": [
        ...
        "three identical musketeer soldiers standing in a staggered line, each with both hands on his own musket held upright against his own shoulder with its fixed bayonet pointing straight up, plain dark blue wool coats with brass buttons over plain white waistcoats, tall black shako hats",      # 線列步兵 (v3: v2's defect was SELF-INFLICTED, not inherited — "a rank of" + "held level" put three identical horizontal barrels on one line and the model merged them into a single barrel spanning all three figures, at BOTH denoise 0.55 and 0.70 (so not parent-borne). Stagger the line and stand the muskets upright so three bayonets cannot be collinear. v2: e3 audit — fill both hands and the torso so e3's kite shield and mail have no slot to persist in)
        "three identical soldiers in olive drab uniforms and steel helmets with rifles, seated together inside an open-topped armored truck with a single white circle painted on its door",  # 摩托化步兵
    ...
    "archers": [
        ...
        "three identical skirmisher soldiers in plain green jackets and soft black peaked caps, kneeling with both hands on long straight muskets aimed forward, powder horns on red shoulder straps",                          # 火槍散兵 (v2: e3 audit — name the headgear slot e3's kettle helmet would otherwise fill, and pin the barrel straight against e3's crossbow limbs)
        "exactly two identical soldiers in hooded leaf-camouflage cloaks, one kneeling aiming a long scoped rifle on a bipod, the other crouching beside him holding a brass spotting scope",     # 狙擊小隊
    ...
    "cavalry": [
        ...
        "two identical hussar cavalrymen on galloping horses with bare heads and plain leather bridles, each swinging one curved saber, plain red jackets with rows of brass braid cords, black fur busby hats",        # 驃騎兵 (v2: e3 audit — e3 horses wear steel face plates; name the bare head + bridle positively so the plate has no slot)
        "a heavy battle tank standing alone on open bare ground with a long cannon barrel and riveted armor plates, painted plain olive drab with a single white circle on the turret side",               # 坦克營
    ...
    "engineers": [
        ...
        "three identical soldiers in plain gray uniforms with red collar trim and soft gray field caps, one carrying a short flat wooden plank bridge section tucked upright under one arm, one shouldering a shovel, one carrying a coil of wire in both hands",        # 工兵營 (v4: v3's "both of his own hands" did NOT stop the plank spanning two figures — a long horizontal object between identical figures merges no matter whose hands are named. Tuck it upright under one arm so it physically cannot reach a neighbour. v3: at denoise 0.70 the e3 standing plank wall cleared 4/4 with framing intact, but 2/4 then SHARED one long plank across two figures' shoulders — the same merge-identical-objects artifact as infantry e4, invariant to denoise. Shorten the plank, anchor it to one figure's own two hands, and give the other two figures their own two-handed loads. v2: e3 audit — name the headgear slot, and lay the planks flat on a shoulder so e3's tall wheeled plank wall cannot stand back up)
        "a yellow armored engineering vehicle with a bulldozer blade and a crane arm and a single white circle on its hull side, two identical soldiers in olive uniforms and steel helmets walking beside it", # 機械化工兵
    ...
    "elite_forces": [
        ...
        "two identical grenadier soldiers in plain dark blue coats and tall black bearskin caps with red cords, each cap front bearing a plain white cloth cockade, each with a plain musket slung on a strap across his back, one holding a round black grenade with a short sputtering fuse, the other shielding that fuse with a cupped gloved hand",     # 擲彈兵 (v2: era-4 audit, MISSED in the first pass because this line starts at era 2 and the audit indexed [3]. Blank bearskin cap fronts bred real-world badges (gold bugle horn, cross pattee) 2/4, and an unprompted bayonet attracted the grenade's flame 2/4 — occupy the cap front with a cockade, give the weapon slot a plain slung musket, and put the fire only at the named fuse)
        "two identical commandos standing one behind the other in black tactical armor with a single plain white circle on each chest plate, night-vision goggles with glowing green lenses, each with both hands on his own compact rifle held angled down",                             # 特種部隊
    ...
    "artillery": [
        ...
        "a tracked self-propelled howitzer standing alone on continuous steel track links with a long elevated gun barrel, painted plain olive drab with a single white circle on the hull side",                   # 自走砲 (v2: inheritance audit — NOTE this line starts at era 2, so this entry is ERA 5, not era 4; its parent is the era-4 field cannon, which has spoked wooden wheels AND two gunners, so occupy the running-gear slot with track links and the crew slot with "standing alone")
    ...
    "bomber": [
        "a red and white striped hot air balloon with a wicker basket, exactly two identical crew standing side by side in the basket, each dropping one small round black bomb over the side",                           # 熱氣球轟炸隊 (v2: this entry is this line's txt2img ROOT — the line starts at era 4, so there is NO parent and no inheritance. Its 3/4 failures were pure count drift (one crew, then three) — pin the count and the pose the way cavalry e3 did)
        "a heavy four-engine propeller bomber aircraft seen alone from the side with four propeller engines along its wings, painted plain olive drab with a single white circle on the fuselage, bomb bay doors open with falling bombs", # 轟炸機聯隊
    ...
    "holy_warriors": [
        ...
        "two identical musketeers in white coats standing close together, each with both hands on his own musket held upright against his own shoulder with the barrel pointing straight up, a single golden tree emblem on each chest, beside a standard bearer gripping the pole of a white banner showing a single large golden tree on one face of the cloth",  # 神權火槍旅 (v3: COMPOUND fix. denoise 0.70 DID break the inherited mace-swing pose (level 4/4, where two rewrites at 0.55 failed 4/4) — so keep this line at 0.70 — but 0.70 then broke framing 3/4 on this wide composition (barrels crossing the right edge, banner finial the top), one seed merged a single barrel across all three figures, and two rendered the tree twice. Standing the muskets upright narrows the silhouette, breaks the collinearity, and pins the tree to one cloth face. v2: era-4 audit, MISSED in the first pass (line starts at era 3). The era-3 flanged mace survived into 4/4 seeds and no musket rendered at all — fill both hands with a concretely described musket (plain wooden stock, straight steel barrel) so the mace has no slot. Banners were clean: single tree, no lettering)
        "a battle tank standing alone on open bare ground painted bone white with a single large golden tree emblem on the turret, golden tassels and small golden tree-shaped charms hanging from the barrel",          # 狂信裝甲師 (e5 v2: "votive ribbons" left an open dangling-charm slot the model filled with a golden crucifix on 0.70 s183 — a real-world religious emblem, the holy_warriors reject class. Name the tree-shaped charm positively so the sanctioned emblem occupies the slot; §14 positive-slot rule)
    ...
    "privateers": [
        ...
        "three identical expedition soldiers in khaki uniforms and pith helmets, a plain red cloth armband of unbroken solid colour on each upper sleeve, with rifles and machetes, one carrying a closed brass-cornered wooden chest with its lid strapped shut, a plain red pennant of unbroken solid cloth on a backpack pole",  # 殖民遠征軍 (v6: v5's leather strap was itself CONVERTED into a red armband bearing a device (chevron / X-cross / star) — the prior does not just resist an occupant, it co-opts one. Every attempt converged on an armband, so name the armband itself in benign form using the phrasing that fixed this line's pennant ("unbroken solid colour"). Human-approved after 4 failed attempts. v5: the sleeve slot OSCILLATED across three attempts — blank gave rank chevrons 3/3, "a single plain white circle" displaced them but grew a red roundel ring 3/3, "solid flat white disc" lost the shape and the chevrons came back 3/3. Painting a mark COMPETES with the insignia prior; it does not remove the canvas. Cover the upper sleeve with a physical object instead so there is no flat area to badge. v4: v3's white circle DID displace the rank chevrons 3/3, but the model then embellished the mark itself into a red-ringed roundel (a real-world insignia pattern) — describe the occupant as a solid flat disc so there is no rim to decorate. v3: v2 fixed both targets (chest closed 3/3, enclosed pennant cutouts gone 2/3) but the insignia prior jumped to the blank SLEEVES as real-world rank chevrons 3/3 — occupy the insignia slot with the style bible's single plain white circle instead of leaving the sleeve blank. v2: era-4 audit, MISSED in the first pass (line starts at era 3). The era-3 gold-overflowing chest rode down as an open lid with spilling coins, and the pennant grew fully enclosed cutouts mid-cloth 2/4 — strap the chest shut and name the pennant cloth unbroken)
        "three identical mercenary contractors standing in a staggered line in tan body armor with a single plain white circle on each chest plate, wraparound sunglasses, each with both hands on his own compact rifle held angled down, one holding a plain metal briefcase",                      # 戰地承包商
    ...
    "shield_wall": [
        ...
        "a chest-high wall of stacked burlap sandbags in even rows with wooden support posts, the sandbag surface soft and rounded throughout, a rifle resting on the top edge",                                                   # 沙包工事 (v2: e3 audit — name the surface soft and rounded so e3's crenellated stone masonry has no slot)
        "a squat gray concrete bunker with a narrow horizontal firing slit, plain gray concrete showing form-board plank texture across its face, iron rebar showing at the edges, sandbags piled at its base",                         # 混凝土碉堡
    ...
    "trench": [
        ...
        "a deep trench segment cut in dry packed earth with a sandbag parapet and dry wooden duckboards along its floor, coils of barbed wire strung on angled iron pickets in front",                           # 鐵絲網塹壕 (v2: e3 audit — e3 is a water-filled moat with stone-lined banks; occupy the floor and the banks with dry earth and duckboards)
        "a wide deep anti-tank ditch with sloped concrete walls showing form-board plank texture, rows of gray concrete pyramid blocks lined up in front",                                            # 反戰車壕
    ...
    "anti_air": [
        ...
        "an anti-aircraft flak cannon with exactly one single long barrel angled skyward on a cross-shaped steel mount resting directly on flat bare ground, a ring gunsight, stacked shell crates beside it, painted plain olive drab",  # 高射砲 (v3: twin parallel barrels off one breech twice on chain 82 — pin the barrel count positively. v2: e3 audit — e3 is a round stone tower; seat the mount on bare ground so no masonry base can persist under it)
    ...
    "enemy_weak": [
        ...
        "a lone ruffian in a ragged dark coat and a crumpled hat, one hand holding a worn flintlock pistol with a plain straight barrel and a curved wooden grip, the other hand holding a rusty knife",                                                 # 弱 (v2: era-4 audit called the crossbow-onto-pistol morph a MEDIUM risk and left it unfixed; it landed 2/4 with bow limbs and string fused onto the flintlock — name the pistol's form so the crossbow has no slot) e4
        "a lone thug in a ragged dark jacket and knit cap holding a crude submachine gun with a plain straight barrel and a taped box magazine",                                                     # 弱 e5
        "a lone looter in a ragged dark coat with a few plain dull metal studs on the chest and a cracked orange visor, holding a sparking makeshift energy pistol",                                                 # 弱 e6 (audit: the blank coat chest is a surface magnet — drew a red CND symbol then yellow/green graffiti glyphs on chain 82; the clean s181 had benign metal studs there, so name plain studs positively to occupy the chest and starve the glyph prior)
    ],
    "enemy_mid": [
        ...
        "two identical renegade soldiers in ragged dark gray uniforms and battered peaked caps standing close together, each with both hands on his own bayoneted musket held upright against his shoulder, mismatched armor plates strapped over their chests",                   # 中 (v3: at denoise 0.70 the e3 kite shield cleared 2/2 but the horizontal musket crossed the right canvas edge on 2 seeds — stand it upright to narrow the silhouette (same geometry fix as infantry e4). v2: e3 audit — name the headgear slot and fill both hands so e3's kite shield and battleaxe have nowhere to go)
        "two identical militia gunmen standing one behind the other in dark body armor and balaclavas, each with both hands on his own rifle held angled down, orange bandanas tied on their arms",                                          # 中 e5
        "two identical rogue soldiers in matte black powered vests with glowing orange visor slits, a single plain orange band painted across each shoulder pad, standing one behind the other, each with both hands on his own heavy energy rifle held angled down",                                         # 中 e6 (audit v3: "smooth unbroken plain pad" FAILED 3/3 again — the orange enemy accent is a stable surface prior that migrates rather than clearing (caret, S-scribble, red roundel), the §14 era-4 migrating-prior case. Stop fighting it: occupy the pad with a benign SANCTIONED mark — a single plain orange band, the same simple horizontal shape enemy_hard's turret stripe uses cleanly — so the prior has no room for a glyph)
    ],
    "enemy_hard": [
        ...
        "a hulking soldier in a heavy dark iron cuirass painted with a single plain white circle and a smooth rounded steel helmet, both hands carrying a hand-cranked multi-barrel gun, ammunition belts across his chest",                                # 硬 (v3: at denoise 0.70 the e3 spiked greathelm and flail cleared 2/2 with framing intact, but one seed then grew a gold ring-and-flame badge on the blank pauldron — the third blank-surface insignia hit of this round (elite_forces cap, privateers sleeve, this) — so occupy the slot with the style bible's single plain white circle. v2: e3 audit — e3 wears a spiked greathelm and swings a flail; name a smooth helmet and occupy both hands)
        "a heavy dark battle tank standing alone on open bare ground with a rusted hull, welded scrap armor plates and a single orange stripe painted on the turret",                                   # 硬 e5
