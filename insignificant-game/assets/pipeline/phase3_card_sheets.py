...
def main() -> None:
    ...
    with open(STATE) as f:
        state = json.load(f)
    ids = [c for c in (PILOT if mode == "pilot" else list(CARDS)) if c in state]
    if not ids:
        raise SystemExit("no generated cards found in state for this mode")
    ...
    d.text((PAD, 14),
           f"Phase 3 cards [{mode}] — one row per subject, seed sweep across columns (pick one seed per row)",
           fill=(255, 255, 255), font=title_font)
    for row, card_id in enumerate(ids):
        y = HDR + (CELL_H + LABEL_H) * row
        label = card_id[len("card_"):]
        d.text((PAD, y + 12), label, fill=(240, 240, 240), font=row_font)
        for col, seed in enumerate(sorted(state[card_id], key=int)):
            stem = state[card_id][seed]["stem"]
            sheet.paste(cell_image(stem, font), (ROWLBL + CELL_W * col, y))
    out = f"../contact-sheets/phase3_cards_{mode}.png"
    sheet.save(out)
