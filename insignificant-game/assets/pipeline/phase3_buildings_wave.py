# phase3_buildings_wave.py — era-gated building generation (cookbook §7). Pilot finding
# (§14 2026-07-12): artifacts propagate down img2img lineage chains, so each era wave is
# §8-reviewed before it seeds the next. State lives in phase3_building_chains.json:
# line -> chain (base seed) -> era -> {"stem": ..., "seed": ...}; a re-roll overwrites its cell
# with a bumped seed and downstream eras then chain from it.
# Usage (ComfyUI venv python, from assets/pipeline/):
#   phase3_buildings_wave.py <era>                          # fill era cells for all lines
#   phase3_buildings_wave.py <era> --redo line:chain:seed   # re-roll listed cells, then regrid
import json
import os
import subprocess
import sys
...
def gen_cell(state: dict, line: str, chain: int, era: int, seed: int) -> None:
    ...
    stem = f"p3_bld_{line}_e{era}_s{seed}"
    cmd = [sys.executable, "comfy_run.py",
           "--seed", str(seed), "--prompt", core + SUFFIX,
           "--prefix", f"phase3-buildings/{stem}", *LORA_ARGS]
    if era == start:
        cmd[2:2] = [T2I]
